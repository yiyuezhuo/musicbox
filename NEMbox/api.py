...
def aesEncrypt(text, secKey):
    ...
    text = encode(text) + encode(pad * chr(pad))
...
def rsaEncrypt(text, pubKey, modulus):
    ...
    rs = pow(int(codecs.encode(encode(text),'hex'), 16), int(pubKey, 16), int(modulus, 16))
...
def createSecretKey(size):
    ...
    '''
    return (
        ''.join(map(lambda xx: (hex(xx)[2:]), os.urandom(size))))[0:16]
...
class NetEase(object):
    ...
    def songs_detail_new_api(self, music_ids, bit_rate=320000):
        ...
        connection = self.session.post(action,
                                       ...
                                       headers=self.header, )
        result = json.loads(connection.content.decode('utf8'))
        return result['data']
